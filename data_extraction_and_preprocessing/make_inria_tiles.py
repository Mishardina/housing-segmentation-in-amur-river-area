...
def augment_image_and_mask(image_path, mask_path, image_name, tiled_images_path, tiled_masks_path):
    ...
    for i in range(len(tiles)):
        cv2.imwrite(os.path.join(tiled_images_path, f'{image_name}_image_{i}.png'), img=tiles[i][0])
...
for subset in ['train']:
    ...
    _ = Parallel(n_jobs=CPU_COUNT)(delayed(augment_image_and_mask)(df_inria.iloc[i]['images_paths'], df_inria.iloc[i]['masks_paths'], df_inria.iloc[i]['images_names'], INRIA_TILED_IMAGES_PATH, INRIA_TILED_MASKS_PATH) for i in tqdm(range(len(df_inria))))
